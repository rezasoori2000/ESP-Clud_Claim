...
const get_API_AdminSettings = () => async (dispatch, getState) => {
  var response = {};
  if (
    Object.entries(getState().adminSettings).length === 0 ||
    getState().adminSettingsChanged
  ) {
    const res = await axios.get(`${props.apiUrl}adminSettings`);
    response = JSON.parse(res.data);
  } else response = getState().adminSettings;
